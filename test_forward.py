...
# Simple test for forward projection (dots)
scene = Scene3D()
...
# Test dots pattern with forward projection
pattern = projector.create_dot_pattern(dot_spacing=40, dot_size=3)
print(f"Dot pattern shape: {pattern.shape}, nonzero: {np.sum(pattern > 0)}")
...
plt.imsave('output/test_forward_dots.png', rgb)
print("Saved to output/test_forward_dots.png")
